...
for t in range(len(sample)):
  ...
  recurrent_z, recurrent_r, recurrent_h = tf.split(matrix_inner, [units, units, -1], axis=-1)
  ...
  if reset_after:
    ...
    recurrent_h = np.dot(r * Hts[-1], recurrent_kernel[:, 2 * units:])

  hh = tanh(x_h + recurrent_h)
  h = z * Hts[-1] + (1 - z) * hh
  # Y = np.dot(h, W_hq) + b_q
  Hts.append(h)
  Y = np.dot(h, W_hq) + b_q
